...
class Game {
    ...
    static class Data {
        ...
        static void transfer_prov_conquest(Prov_Id prov_id, int new_owner_id, int new_armies_num) {
            ...
            player_data[new_owner_id].owned_provs.add(prov_id);
            set_num_armies(prov_id, new_armies_num);
        }

        static void init_test_gamedata() {
            basic_load(3);
            for (int i = 0; i < prov_data.length; i++) {
                //divides the provs among the players and gives each prov 5 armies
                if (i == 0) {
                    player_data[i].owned_provs.add(new Prov_Id(i));
                } else {
                    player_data[i % no_players].owned_provs.add(new Prov_Id(i));
                }
                prov_data[i].no_armies = 5;
            }
        }

        static void attack_a_round(Prov_Id prov_from, Prov_Id prov_to/*, int no_armies*/) {
            ...
            Pair defense_roll = defensive_roll(get_num_armies(prov_to));
        ...
        static int get_num_armies(Prov_Id prov_id) {
            return prov_data[prov_id.to_int()].no_armies;
        }


        static void tmp_reinforce() {//TODO not final class just for testing
            for (Prov_Id owned : player_data[active_player].owned_provs) {
                prov_data[owned.to_int()].no_armies += 3;
            }
        ...
        private static void set_num_armies(Prov_Id target_prov, int num_armies) {
            prov_data[target_prov.to_int()].no_armies = num_armies;
        }
        ...
        private static void add_armies(Prov_Id target_prov_id, int num_armies) {
            if (get_num_armies(target_prov_id) < 0) {
                prov_data[target_prov_id.to_int()].no_armies = 0;
            }
            prov_data[target_prov_id.to_int()].no_armies += num_armies;
        }

        private static Pair defensive_roll(int num_armies) {//TODO allow the player to choose how many dice to use
            assert num_armies > 0;
        ...
        private static void delete_ownership(Prov_Id prov_id) {
            int removed_counter = 0;
            for (Player_Data player : player_data) {
                for (int i = 0; i < player.owned_provs.size(); i++) {
                    if (player.owned_provs.get(i) == prov_id) {
                        player.owned_provs.remove(i);
                    }
                }
            }
        ...
        private static class Player_Data {
            int player_id;
            ArrayList<Prov_Id> owned_provs;

            Player_Data(int player_id_arg) {
                player_id = player_id_arg;
                owned_provs = new ArrayList<Prov_Id>();
            }

            Player_Data(int player_id_arg, ArrayList<Prov_Id> owns_provs_arg) {
                player_id = player_id_arg;
                owned_provs = owns_provs_arg;
            }
        }
    ...
    public static class State {
        ...
        static int game_state = 1;//TODO implement setup
        ...
        static class Turn_State{

            static String get_turn_phase_name(){
                switch (turn_state) {
                    case -1:
                        return "setup phase";
                    case 0:
            ...
            static void advance_turn_state() {//keeps track of the turn phase
                // Increments state
                if (turn_state< 2) {
                    turn_state++;
                    update();
